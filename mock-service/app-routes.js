          ...
          if (def.auth) {
            actions.push((req, res, next) => {
              passport.authenticate(def.auth, (err, user) => {
                if (err || !user) {
                  next((err instanceof UnauthorizedError) ? err : new ForbiddenError('You are not allowed to perform this action!'));
                } else {
            ...
            actions.push((req, res, next) => {
              if (!req.user) {
                next(new ForbiddenError('Action is not allowed for anonymous!'));
              }
